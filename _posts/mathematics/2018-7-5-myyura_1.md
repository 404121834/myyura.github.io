...
并不是些什么故弄玄虚的符号，但实际上在专业的数学读物上比较少见，不过Knuth使用这些
符号约定，考虑面向大都是计算机相关背景的读者，因而选取他老人家的符号约定也在情理之中。
...
$k$个因子$x$与$n-k$个因子$y$的项的个数，这恰好是从$n$个$(x+y)$的因子中选取k个的方法数--他们都
提供一个x。
...
注意到这个式子的组合学解释，它的每一项都代表了从$s$项物体中选取$n-k$项，并从$t$项物体中
选取k项的方法数，这实际上也就是从$s+t$项中选取$n$项的方法数(考虑将n项分别分配在s和t中
并求和)，即有
...
但是不得不说，很多时候它还是挺有用的。
由范德蒙德卷积(以下简称卷积)我们可以推导出一些有趣的结论，譬如当$s=t=n$的时候，则有:
...
转之前于下指标中创造一个$r-k$。对，就是这样，我们通过对称来做到这一点。

  $$
  \begin{align}
  \sum_{k=0}^r {r-k \choose m} {s \choose k-t} (-1)^{k-t} &= \sum_{k=0}^r {r-k \choose r-k-m} {s \choose k-t} (-1)^{k-t} \\
